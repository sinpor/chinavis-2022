...
export const CoreBar: React.FC = observer(() => {
	const industryName = {
		A: '涉黄',
		B: '涉赌',
		C: '诈骗',
	...
	const updateCoreBar = useCallback((nodeIds: any[], series: any[], coreNodes: any[]) => {
		...
		barChart.on('mouseover', function(params) {
			...
			for (const node of coreNodes) {
				if (node.id === parseInt(params.name.split('-')[0])) {
					const nodesover = node.linkNodeIds;
	...
	useEffect(() => {
		updateCoreBar(coreIndustry.nodeIds, coreIndustry.series, coreIndustry.coreNodes);
	}, [coreIndustry]);
